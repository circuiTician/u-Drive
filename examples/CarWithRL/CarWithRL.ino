    ...
    case WStype_TEXT:
      {
        ...
        if (!error) {
          ...
          // --- 3. Create file  ---
          else if (strcmp(msgType, "create_file") == 0) {
            fName = doc["name"].as<String>();
            fName = "/" + fName + ".csv";
            const char* file_Name = fName.c_str();
            if (uD.createCSVWithHeader(file_Name, "timeStamp", "LEFT", "MIDDLE", "RIGHT", "LEFT_M", "RIGHT_M")) Serial.println(fName +" Created");
          }
...
void loop() {
  ...
  if (millis() - lastStoreTime > 1115 && CNTD) {
    const char* file_Name = fName.c_str();
    uD.appendCSV(file_Name, ((millis() - startTime)/1000), uD.getDistance(LEFT), uD.getDistance(MIDDLE), uD.getDistance(RIGHT), leftSpeed, rightSpeed);
